'''
    Descripttion: 加载数据集的工具类
    version: 1.0
    Author: Geek
    Date: 2022-08-24 09:54:31
LastEditors: Geek
LastEditTime: 2022-09-16 16:03:49
'''
import os
from torch.utils.data import DataLoader, Dataset
import random
import json
import matplotlib.pyplot as plt
from PIL import Image
import torch
import numpy as np
from torchvision import transforms
import cv2
from tqdm import tqdm


def read_split_data(root: str, val_rate: float = 0.2, plot_image=False):
    """
        功能: 划分数据集
        生成: 类别的json文件、样本数量柱状图(plot_image=True)
    """
    random.seed(0)  # 保证随机结果可复现
    assert os.path.exists(root), "dataset root: {} does not exist.".format(root)

    # 遍历文件夹，一个文件夹对应一个类别
    data_class = [cla for cla in os.listdir(root) if os.path.isdir(os.path.join(root, cla))]
    # 排序，保证顺序一致
    data_class.sort()
    # 生成类别名称以及对应的数字索引
    class_indices = dict((v, k) for v, k in enumerate(data_class))
    json_str = json.dumps(class_indices, indent=4)
    with open('class_indices.json', 'w') as json_file:
        json_file.write(json_str)

    # 翻转 class_indices , 以便后面获取序号
    class_indices = dict((k,v) for v,k in enumerate(data_class))

    train_images_path = []  # 存储训练集的所有图片路径
    train_images_label = []  # 存储训练集图片对应索引信息
    val_images_path = []  # 存储验证集的所有图片路径
    val_images_label = []  # 存储验证集图片对应索引信息
    every_class_num = []  # 存储每个类别的样本总数
    # 遍历每个文件夹下的文件
    for cla in data_class:
        cla_path = os.path.join(root, cla)
        # 遍历获取所有文件路径
        images = [os.path.join(root, cla, i) for i in os.listdir(cla_path)]
        # 获取该类别对应的索引
        image_class = class_indices[cla]
        # 记录该类别的样本数量
        every_class_num.append(len(images))
        # 按比例随机采样验证样本
        val_path = random.sample(images, k=int(len(images) * val_rate))

        for img_path in images:
            if img_path in val_path:  # 如果该路径在采样的验证集样本中则存入验证集
                val_images_path.append(img_path)
                val_images_label.append(image_class)
            else:  # 否则存入训练集
                train_images_path.append(img_path)
                train_images_label.append(image_class)

    print("{} images were found in the dataset.".format(sum(every_class_num)))
    print("{} images for training.".format(len(train_images_path)))
    print("{} images for validation.".format(len(val_images_path)))

    if plot_image:
        # 绘制每种类别个数柱状图
        plt.bar(range(len(data_class)), every_class_num, align='center')
        # 将横坐标0,1,2,3,4替换为相应的类别名称
        plt.xticks(range(len(data_class)), data_class)
        # 在柱状图上添加数值标签
        for i, v in enumerate(every_class_num):
            plt.text(x=i, y=v + 5, s=str(v), ha='center')
        # 设置x坐标
        plt.xlabel('image class')
        # 设置y坐标
        plt.ylabel('number of images')
        # 设置柱状图的标题
        plt.title('class distribution')
        plt.savefig('datasets_info.png', dpi=100)
    return train_images_path, train_images_label, val_images_path, val_images_label


def read_splited_data(root: str, plot_image=False):
    """
        功能: 对已划分完的数据进行统计
              划分完的目录结构如下:
                root
                    train
                        0
                        1
                        ...
                    val
                        0
                        1
                        ...
        生成: 类别的json文件、样本数量柱状图(plot_image=True)
    """
    assert os.path.exists(root), "dataset root: {} does not exist.".format(root)

    train_path = os.path.join(root, 'train')
    val_path = os.path.join(root, 'val')
    

    # 遍历文件夹，一个文件夹对应一个类别
    data_class = [cla for cla in os.listdir(train_path) if os.path.isdir(os.path.join(train_path, cla))]
    # 排序，保证顺序一致
    data_class.sort()
    # 生成类别名称以及对应的数字索引
    class_indices = dict((v, k) for v, k in enumerate(data_class))
    json_str = json.dumps(class_indices, indent=4)
    with open('class_indices.json', 'w') as json_file:
        json_file.write(json_str)

    # 翻转 class_indices , 以便后面获取序号
    class_indices = dict((k,v) for v,k in enumerate(data_class))

    train_images_path = []  # 存储训练集的所有图片路径
    train_images_label = []  # 存储训练集图片对应索引信息
    val_images_path = []  # 存储验证集的所有图片路径
    val_images_label = []  # 存储验证集图片对应索引信息
    every_class_num = [0,0,0,0,0]  # 存储每个类别的样本总数

    # 训练集
    for cla in os.listdir(train_path):
        cla_path = os.path.join(train_path, cla)
        image_class = class_indices[cla]
        images = [os.path.join(train_path, cla, i) for i in os.listdir(cla_path)]
        every_class_num[image_class] = every_class_num[image_class] + len(images)
        for img_path in images:
            train_images_path.append(img_path)
            train_images_label.append(image_class)

    j = 0
    # 测试集
    for cla in os.listdir(val_path):
        cla_path = os.path.join(val_path, cla)
        image_class = class_indices[cla]
        images = [os.path.join(val_path, cla, i) for i in os.listdir(cla_path)]
        every_class_num[image_class] = every_class_num[image_class] + len(images)
        for img_path in images:
            val_images_path.append(img_path)
            val_images_label.append(image_class)

    # print("{} images were found in the dataset.".format(sum(every_class_num)))
    # print("{} images for training.".format(len(train_images_path)))
    # print("{} images for validation.".format(len(val_images_path)))

    if plot_image:
        # 绘制每种类别个数柱状图
        plt.bar(range(len(data_class)), every_class_num, align='center')
        # 将横坐标0,1,2,3,4替换为相应的类别名称
        plt.xticks(range(len(data_class)), data_class)
        # 在柱状图上添加数值标签
        for i, v in enumerate(every_class_num):
            plt.text(x=i, y=v + 5, s=str(v), ha='center')
        # 设置x坐标
        plt.xlabel('image class')
        # 设置y坐标
        plt.ylabel('number of images')
        # 设置柱状图的标题
        plt.title('class distribution')
        plt.savefig('datasets_info.png', dpi=100)
    return train_images_path, train_images_label, val_images_path, val_images_label

class MyDataSet(Dataset):
    """自定义数据集"""
    def __init__(self, images_path: list, images_class: list, transform=None):
        self.images_path = images_path
        self.images_class = images_class
        self.transform = transform

    def __len__(self):
        return len(self.images_path)

    def __getitem__(self, item):
        img = Image.open(self.images_path[item])
        # RGB为彩色图片，L为灰度图片
        if img.mode != 'RGB':
            raise ValueError("image: {} isn't RGB mode.".format(self.images_path[item]))
        label = self.images_class[item]
        if self.transform is not None:
            img = self.transform(img)
        # print(self.images_path[item])
        # 获得图片坐标信息
        image_path = self.images_path[item]
        begin_index = image_path.find("^x")
        image_path = image_path[begin_index:]
        s = image_path.split("_")
        location = (int(s[1][:-1]),int(s[2][:-1]),int(s[3][:-1]),int(s[4][:-4]))
        return img, label, location

def plot_class_preds(net,
                     images_dir: str,
                     transform,
                     num_plot: int = 5,
                     device=0):
    if not os.path.exists(images_dir):
        print("not found {} path, ignore add figure.".format(images_dir))
        return None

    label_path = os.path.join(images_dir, "label.txt")
    if not os.path.exists(label_path):
        print("not found {} file, ignore add figure".format(label_path))
        return None

    # read class_indict
    json_label_path = './class_indices.json'
    assert os.path.exists(json_label_path), "not found {}".format(json_label_path)
    json_file = open(json_label_path, 'r')
    # {"0": "daisy"}
    data_class = json.load(json_file)
    # {"daisy": "0"}
    class_indices = dict((v, k) for k, v in data_class.items())

    # reading label.txt file
    label_info = []
    with open(label_path, "r") as rd:
        for line in rd.readlines():
            line = line.strip()
            if len(line) > 0:
                split_info = [i for i in line.split(" ") if len(i) > 0]
                assert len(split_info) == 2, "label format error, expect file_name and class_name"
                image_name, class_name = split_info
                image_path = os.path.join(images_dir, image_name)
                # 如果文件不存在，则跳过
                if not os.path.exists(image_path):
                    print("not found {}, skip.".format(image_path))
                    continue
                # 如果读取的类别不在给定的类别内，则跳过
                if class_name not in class_indices.keys():
                    print("unrecognized category {}, skip".format(class_name))
                    continue
                label_info.append([image_path, class_name])

    if len(label_info) == 0:
        return None

    # get first num_plot info
    if len(label_info) > num_plot:
        label_info = label_info[:num_plot]

    num_imgs = len(label_info)
    images = []
    labels = []
    for img_path, class_name in label_info:
        # read img
        img = Image.open(img_path).convert("RGB")
        label_index = int(class_indices[class_name])

        # preprocessing
        img = transform(img)
        images.append(img)
        labels.append(label_index)

    # batching images
    images = torch.stack(images, dim=0).to(device)

    # inference
    with torch.no_grad():
        output = net(images)
        # 返回最大值和索引
        probs, preds = torch.max(torch.softmax(output, dim=1), dim=1)
        probs = probs.cpu().numpy()
        preds = preds.cpu().numpy()

    # width, height
    fig = plt.figure(figsize=(num_imgs * 2.5, 3), dpi=100)
    for i in range(num_imgs):
        # 1：子图共1行，num_imgs:子图共num_imgs列，当前绘制第i+1个子图
        ax = fig.add_subplot(1, num_imgs, i+1, xticks=[], yticks=[])

        # CHW -> HWC
        npimg = images[i].cpu().numpy().transpose(1, 2, 0)

        # 将图像还原至标准化之前
        # mean:[0.485, 0.456, 0.406], std:[0.229, 0.224, 0.225]
        npimg = (npimg * [0.229, 0.224, 0.225] + [0.485, 0.456, 0.406]) * 255
        plt.imshow(npimg.astype('uint8'))

        title = "{}, {:.2f}%\n(label: {})".format(
            data_class[str(preds[i])],  # predict class
            probs[i] * 100,  # predict probability
            data_class[str(labels[i])]  # true class
        )
        ax.set_title(title, color=("green" if preds[i] == labels[i] else "red"))
    return fig


# def plot_gram_cam_class_preds(net,
#                      images_dir: str,
#                      transform,
#                      num_plot: int = 5,
#                      device=0):
#     """绘制热力图图像"""
#     # 多GPU需要使用 net.module 来获取原始模型
#     cam = GradCAM(model=net, target_layers=[net.module._conv_head], use_cuda=True)    
    
#     if not os.path.exists(images_dir):
#         print("not found {} path, ignore add figure.".format(images_dir))
#         return None

#     label_path = os.path.join(images_dir, "label.txt")
#     if not os.path.exists(label_path):
#         print("not found {} file, ignore add figure".format(label_path))
#         return None

#     # read class_indict
#     json_label_path = './class_indices.json'
#     assert os.path.exists(json_label_path), "not found {}".format(json_label_path)
#     json_file = open(json_label_path, 'r')
#     # {"0": "daisy"}
#     data_class = json.load(json_file)
#     # {"daisy": "0"}
#     class_indices = dict((v, k) for k, v in data_class.items())

#     # reading label.txt file
#     label_info = []
#     with open(label_path, "r") as rd:
#         for line in rd.readlines():
#             line = line.strip()
#             if len(line) > 0:
#                 split_info = [i for i in line.split(" ") if len(i) > 0]
#                 assert len(split_info) == 2, "label format error, expect file_name and class_name"
#                 image_name, class_name = split_info
#                 image_path = os.path.join(images_dir, image_name)
#                 # 如果文件不存在，则跳过
#                 if not os.path.exists(image_path):
#                     print("not found {}, skip.".format(image_path))
#                     continue
#                 # 如果读取的类别不在给定的类别内，则跳过
#                 if class_name not in class_indices.keys():
#                     print("unrecognized category {}, skip".format(class_name))
#                     continue
#                 label_info.append([image_path, class_name])

#     if len(label_info) == 0:
#         return None

#     # get first num_plot info
#     if len(label_info) > num_plot:
#         label_info = label_info[:num_plot]

#     num_imgs = len(label_info)
#     images = []
#     labels = []
#     grad_cam_images = []
#     for img_path, class_name in label_info:
#         # read img
#         img = Image.open(img_path).convert("RGB")
#         label_index = int(class_indices[class_name])

#         # preprocessing
#         img = transform(img)
#         images.append(img)

#         img = Image.open(img_path).convert("RGB")
#         img = np.array(img, dtype=np.uint8)
#         grad_cam_images.append(img)
#         labels.append(label_index)

#     # batching images
#     images = torch.stack(images, dim=0).to(device)
    
#     visualization_list = []
#     data_transform = transforms.Compose([transforms.ToTensor(),
#                                          transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    
    
#     # 生成热力图
#     for index, grad_cam_img in enumerate(grad_cam_images):
#         input_tensor = data_transform(grad_cam_img)
#         # 扩充维度 (batch_size, W,H,C)
#         input_tensor = torch.unsqueeze(input_tensor, dim=0)
#         # 设置目标标签
#         target_category = [ClassifierOutputTarget(labels[index])]
        
#         grayscale_cam = cam(input_tensor=input_tensor, targets=target_category)
#         grayscale_cam = grayscale_cam[0, :]
#         visualization = show_cam_on_image(grad_cam_img.astype(dtype=np.float32) / 255.,
#                                         grayscale_cam,
#                                         use_rgb=True)
#         visualization_list.append(visualization)

#     # inference
#     with torch.no_grad():
#         output = net(images)        
#         # 返回最大值和索引
#         probs, preds = torch.max(torch.softmax(output, dim=1), dim=1)
#         probs = probs.cpu().numpy()
#         preds = preds.cpu().numpy()

#     # width, height
#     fig = plt.figure(figsize=(num_imgs * 2.5, 3), dpi=100)
#     for i in range(num_imgs):
#         # 1：子图共1行，num_imgs:子图共num_imgs列，当前绘制第i+1个子图
#         ax = fig.add_subplot(1, num_imgs, i+1, xticks=[], yticks=[])
        
#         # 拼接热力图和原图
#         npimg = np.vstack((visualization_list[i], grad_cam_images[i]))

#         plt.imshow(npimg.astype('uint8'))
        
#         title = "{}, {:.2f}%\n(label: {})".format(
#             data_class[str(preds[i])],  # predict class
#             probs[i] * 100,  # predict probability
#             data_class[str(labels[i])]  # true class
#         )
#         ax.set_title(title, color=("green" if preds[i] == labels[i] else "red"))
#     return fig