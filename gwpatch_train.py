...
def evaluate(model, graph_diff, g_same, features, labels, mask, patch_img1, patch_img2):
    model.eval()
    with torch.no_grad():
        logits = model(graph_diff, g_same, features, patch_img1, patch_img2)
        logits = logits[mask]
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for wsi_path in wsi_paths:
            ...
            graph_save_path_diff = os.path.join(workplace, wsi_path, "graph_diff.dgl")
            graph_save_path_same = os.path.join(workplace, wsi_path, "graph_same.dgl")
            ...
            graphs_diff, _ = dgl.load_graphs(f'{graph_save_path_diff}')
            graphs_same, _ = dgl.load_graphs(f'{graph_save_path_same}')
            g_diff = graphs_diff[0]
            g_same = graphs_same[0]
            ...
            train_mask = torch.zeros(g_diff.num_nodes(), dtype=torch.bool)
            val_mask = torch.zeros(g_diff.num_nodes(), dtype=torch.bool)
            train_mask[:len(train_idx)] = True
            val_mask[len(train_idx):] = True
            node_labels = labels.squeeze()
            g_diff = g_diff.to(device)
            g_same = g_same.to(device)
            node_labels = g_diff.ndata['label'].squeeze()
            node_features = g_diff.ndata['feat']
            ...
            model.train()
            opt.zero_grad()
            logits = model(g_diff, g_same, node_features, patch_img1, patch_img2)
            loss = F.cross_entropy(logits[train_mask], node_labels[train_mask])
            loss.backward()
            opt.step()
            acc, current_num, val_num = evaluate(model, g_diff, g_same, node_features, node_labels, val_mask, patch_img1, patch_img2)
        ...
        if now_acc>best_acc:
            best_acc = now_acc
            torch.save(model.state_dict(), f'/home75/lichaowei/deeplearning/classification/GNN/timm_gnn/chaocan/best.pth')
        torch.save(model.state_dict(), f'/home75/lichaowei/deeplearning/classification/GNN/timm_gnn/chaocan/last.pth')
